...
- **product_name_cache** - Кэш переводов названий продуктов (NEW!)
...
### OpenAI
- `OpenAIService` - сервис для распознавания чеков (server-side)
- `parseReceiptImage()` - парсинг изображения чека (via API)
...
3. AI автоматически распознает:
   - Названия продуктов (с оригинальным названием с чека)
...
**Новое:** Система кэширования переводов обеспечивает консистентность:
- Один и тот же товар всегда получает одинаковый перевод
- Используется единая категория для группировки похожих продуктов
- Не создаются дубликаты при повторных покупках

Подробная документация: 
- [RECEIPT_SCANNING.md](./RECEIPT_SCANNING.md)
- [PRODUCT_NAME_CACHE.md](./PRODUCT_NAME_CACHE.md) - Система кэширования переводов
...
- [PRODUCT_NAME_CACHE.md](./PRODUCT_NAME_CACHE.md) - Система кэширования переводов
- [APPLY_CACHE_UPDATE.md](./APPLY_CACHE_UPDATE.md) - Инструкция по применению обновления

### Миграции
- `database_schema.sql` - Основная схема БД
- `migration_add_product_name_cache.sql` - Добавление кэша переводов
- `migration_add_product_type.sql` - Добавление типов продуктов
- `migration_add_original_name.sql` - Добавление оригинальных названий
- `test_product_cache.sql` - Тесты кэша переводов
