# Гарантированное исправление (V2)

Так как ошибка не уходит, значит база данных "запомнила" старую версию кода. Мы создадим **новую версию функции (v2)** и переключим триггер на нее.

1. Откройте **SQL Editor** в Supabase Dashboard.
2. Скопируйте и выполните код ниже.

```sql
-- FORCE FIX: Use a completely new function name to ensure old code is abandoned.
-- Description: Creates handle_new_user_v2 and points the trigger to it.

-- 1. Create the NEW function (V2)
...
BEGIN
  ...
  RAISE LOG 'handle_new_user_v2 triggered for email: %', v_user_email;
  ...
  EXCEPTION WHEN OTHERS THEN
      ...
      RAISE LOG 'CRITICAL ERROR in handle_new_user_v2 (swallowed): % %', SQLERRM, SQLSTATE;
  END;
...
-- 2. Clean up OLD trigger and function
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- 3. Create NEW trigger pointing to V2 function
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user_v2();
```

После выполнения этого скрипта, попробуйте войти снова. **Обязательно убедитесь, что в URL нет старого `?error=...` (откройте чистую вкладку или удалите хвост URL).**
