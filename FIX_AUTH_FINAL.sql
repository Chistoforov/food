-- Fix Auth Trigger Logic (Final Version)
-- Description: Fixes potential search_path issues and adds robust error handling

CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER 
SECURITY DEFINER -- Run as owner (postgres) to bypass RLS/permissions
SET search_path = public -- Explicitly set search path to find tables
AS $$
DECLARE
  v_family_id INTEGER;
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
      ...
      -- Re-raise to abort transaction (Auth service will see "Database error saving new user")
